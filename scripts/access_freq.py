...
print (df_usa_mening['NEIS1279'].value_counts(normalize=True, dropna=True))

# store the frequency counts of a loci into dictionary 'counts'
counts = df_usa_mening['NEIS1279'].value_counts(normalize=True, dropna=True).to_dict()
pprint(counts)
...
plt.scatter(data["x"], data["y"], marker = 'o', c=t)
...
#look at all data
for c in df_usa_mening.columns:
    print ("---- %s ---" % c)
    print (df_usa_mening[c].value_counts(normalize=True, dropna=True))

#turn all values in dataframe into strings
df_usa_menings = df_usa_mening.applymap(str)
